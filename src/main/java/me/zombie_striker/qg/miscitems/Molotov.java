...
//        }.runTaskTimer(QAMain.getInstance(), 5 * 20, 10));
        ...
        h.setTimer(QAMain.mybukkit.runTaskTimer(((Player) h.getHolder()), null, null, () -> {
            ...
            if (k.get() == 40) {
                ...
                for (Entity ej : h.getHolder().getNearbyEntities(radius, radius, radius))
                    if (ej instanceof LivingEntity) {
                        QAMain.DEBUG("Firedamage to " + ej.getName());
                        try {
                            if (ProtectionHandler.canPvp(ej.getLocation())) {
                                ej.setFireTicks(20);
                            }
                        } catch (Error error) {
                            ej.setFireTicks(20);
