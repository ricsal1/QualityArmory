...
//        }.runTaskLater(QAMain.getInstance(), 5 * 20));
        ...
        h.setTimer(QAMain.mybukkit.runTaskLater(((Player) h.getHolder()), null, null, () -> {
            ...
            try {
                for (Entity ez : h.getHolder().getNearbyEntities(radius, radius, radius)) {
                    if (ez instanceof LivingEntity) {
                        QAMain.DEBUG("Flashbaned " + ez.getName());
                        ((LivingEntity) ez).addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, 20 * 10, 2));
                    }
                }
            } catch (Error ei) {
            }
