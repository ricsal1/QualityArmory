...
//        }.runTaskLater(QAMain.getInstance(), 5 * 20));
        ...
        h.setTimer(QAMain.mybukkit.runTaskLater(((Player) h.getHolder()), null, null, () -> {
            if (h.getHolder() instanceof Player) {
                QAMain.DEBUG("Player did not throw. Damaged for " + dmageLevel);
                removeGrenade(((Player) h.getHolder()));
                ((Player) h.getHolder()).damage(dmageLevel);
            ...
            Player thro = Bukkit.getPlayer(h.getOwner());
            try {
                for (Entity ev : h.getHolder().getNearbyEntities(radius, radius, radius)) {
                    if (ev instanceof LivingEntity) {
                        double dam = (dmageLevel / ev.getLocation().distance(h.getHolder().getLocation()));
                        QAMain.DEBUG("Grenade-Damaging " + ev.getName() + " : " + dam + " DAM.");
                        if (thro == null)
                            ((LivingEntity) ev).damage(dam);
                        else
                            ((LivingEntity) ev).damage(dam, thro);
                    }
                }
            } catch (Error ex) {
                h.getHolder().getWorld().createExplosion(h.getHolder().getLocation(), 1);
