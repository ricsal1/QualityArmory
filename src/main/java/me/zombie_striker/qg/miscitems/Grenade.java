...
public class Grenade extends CustomBaseObject implements ThrowableItems {
	...
	public boolean onPull(Player e, ItemStack usedItem) {
		...
		h.setTimer(QAMain.myBukkit.runTaskLater(thrower, null, null, () -> {
			...
			try {
				for (Entity e1 : h.getHolder().getNearbyEntities(radius, radius, radius)) {
					if (e1 instanceof LivingEntity) {
						double dam = (dmageLevel / e1.getLocation().distance(h.getHolder().getLocation()));
						QAMain.DEBUG("Grenade-Damaging " + e1.getName() + " : " + dam + " DAM.");
						if (thro == null)
							((LivingEntity) e1).damage(dam);
						else
							((LivingEntity) e1).damage(dam, thro);
					}
				}
			} catch (Error e2) {
