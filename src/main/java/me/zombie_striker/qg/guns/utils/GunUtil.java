...
//                        }.runTaskLater(QAMain.getInstance(), 3);
    ...
    public static void basicShoot(boolean offhand, final Gun g, final Player player, final double acc, int times, boolean holdingRMB) {
        ...
        AtomicBoolean regularshoot = new AtomicBoolean(true);
        ...
        if (g.getChargingHandler() != null) {
            ...
            regularshoot.set(g.getChargingHandler().shoot(g, player, firstGunInstance));
        }
...
//            }.runTaskTimer(QAMain.getInstance(), 10 / g.getFireRate(), 10 / g.getFireRate()));
            ...
            AtomicInteger slotUsed = new AtomicInteger(player.getInventory().getHeldItemSlot());
            ...
            rapidfireshooters.put(player.getUniqueId(), QAMain.mybukkit.runTaskTimer(player, null, null, () -> {
                ...
                if (((QAMain.SWAP_TO_LMB_SHOOT && player.isSneaking() == QAMain.SwapSneakToSingleFire)) || slotUsed.get() != player.getInventory().getHeldItemSlot() || amount <= 0) {
                    QAMain.mybukkit.cancelTask(rapidfireshooters.remove(player.getUniqueId()));
                    return;
                }

                regularshoot.set(true);
                if (g.getChargingHandler() != null && (!g.getChargingHandler().isCharging(player)
                        && (g.getReloadingingHandler() == null || !g.getReloadingingHandler().isReloading(player)))) {
                    regularshoot.set(g.getChargingHandler().shoot(g, player, temp));
                    QAMain.DEBUG(
                            "Charging (rapidfire) shoot debug: " + g.getName() + " = " + g.getChargingHandler() == null
                                    ? "null"
                                    : g.getChargingHandler().getName());
