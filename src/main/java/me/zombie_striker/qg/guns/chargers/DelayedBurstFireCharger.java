...
public class DelayedBurstFireCharger implements ChargingHandler {
	...
	public boolean shoot(final Gun g, final Player player, final ItemStack stack) {
		...
		final int[] shotCurrently = {1};

		final int[] currentRate = {(int) (10 / g.getFireRate() / Math.pow(2, g.getBulletsPerShot()))};
		final int[] skippedTicks = {0};

		shooters.put(player.getUniqueId(),QAMain.myBukkit.runTaskTimer(player, null, null, () -> {

						if (skippedTicks[0] >= currentRate[0]) {
							skippedTicks[0] = 0;
							currentRate[0] *= 2;
						} else {
							skippedTicks[0]++;
						...
						if (shotCurrently[0] >= g.getBulletsPerShot() || slotUsed != player.getInventory().getHeldItemSlot()
								|| amount <= 0) {
